...
4. **Validate Multi-Model MCP Tools**
   - Check `codeagent-wrapper` availability: `$CLAUDE_PLUGIN_ROOT/bin/run-wrapper --version`
...
5. **Validate Context Retrieval MCP** (Optional)
   - **Check Active Tool**: Is `mcp__fast-context__fast_context_search` available in the current session?
