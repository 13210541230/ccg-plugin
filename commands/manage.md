...
| 测试 | `$CLAUDE_PLUGIN_ROOT/prompts//tester.md` | `$CLAUDE_PLUGIN_ROOT/prompts//tester.md` |
...
调用 mcp__fast-context__fast_context_search 检索与任务相关的代码上下文：
- 使用自然语言构建语义查询
- 禁止基于假设回答
...
Codex-A（逻辑分析）:
Bash({
  command: "$CLAUDE_PLUGIN_ROOT/bin/run-wrapper --lite --backend ${CCG_BACKEND:-codex} - \"$(pwd)\" <<'EOF'
...
Codex-A（后端规划）:
Bash({
  command: "$CLAUDE_PLUGIN_ROOT/bin/run-wrapper --lite --backend ${CCG_BACKEND:-codex} resume {{CODEX_SESSION}} - \"$(pwd)\" <<'EOF'
ROLE_FILE: $CLAUDE_PLUGIN_ROOT/prompts/$CCG_BACKEND/architect.md
<TASK>
需求：{{TASK_CONTENT}}
...
Codex-B（架构规划）:
Bash({
  command: "$CLAUDE_PLUGIN_ROOT/bin/run-wrapper --lite --backend ${CCG_BACKEND:-codex} resume {{CODEX_B_SESSION}} - \"$(pwd)\" <<'EOF'
ROLE_FILE: $CLAUDE_PLUGIN_ROOT/prompts/$CCG_BACKEND/architect.md
<TASK>
需求：{{TASK_CONTENT}}
上下文：{{ANALYZE_FINDINGS}}
...
Bash({
  command: "$CLAUDE_PLUGIN_ROOT/bin/run-wrapper --lite --backend ${CCG_BACKEND:-codex} - \"$(pwd)\" <<'EOF'
ROLE_FILE: $CLAUDE_PLUGIN_ROOT/prompts/$CCG_BACKEND/tester.md
<TASK>
为以下文件生成测试：
{{CHANGED_FILES}}
上下文：{{PROJECT_CONTEXT}}
