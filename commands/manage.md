...
| 测试 | `$CLAUDE_PLUGIN_ROOT/prompts/codex/tester.md` | `$CLAUDE_PLUGIN_ROOT/prompts/codex/tester.md` |
...
Codex-A（逻辑分析）:
Bash({
  command: "$CLAUDE_PLUGIN_ROOT/bin/run-wrapper --lite --backend codex - \"$(pwd)\" <<'EOF'
...
Codex-A（后端规划）:
Bash({
  command: "$CLAUDE_PLUGIN_ROOT/bin/run-wrapper --lite --backend codex resume {{CODEX_SESSION}} - \"$(pwd)\" <<'EOF'
ROLE_FILE: $CLAUDE_PLUGIN_ROOT/prompts/codex/architect.md
<TASK>
需求：{{TASK_CONTENT}}
...
Codex-B（架构规划）:
Bash({
  command: "$CLAUDE_PLUGIN_ROOT/bin/run-wrapper --lite --backend codex resume {{CODEX_B_SESSION}} - \"$(pwd)\" <<'EOF'
ROLE_FILE: $CLAUDE_PLUGIN_ROOT/prompts/codex/architect.md
<TASK>
需求：{{TASK_CONTENT}}
上下文：{{ANALYZE_FINDINGS}}
...
Bash({
  command: "$CLAUDE_PLUGIN_ROOT/bin/run-wrapper --lite --backend codex - \"$(pwd)\" <<'EOF'
ROLE_FILE: $CLAUDE_PLUGIN_ROOT/prompts/codex/tester.md
<TASK>
为以下文件生成测试：
{{CHANGED_FILES}}
上下文：{{PROJECT_CONTEXT}}
