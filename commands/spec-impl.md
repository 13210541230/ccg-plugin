...
4. **Route Tasks to Appropriate Model**
   ...
   **工作目录**：`{{WORKDIR}}` 替换为目标工作目录的绝对路径。如果用户通过 `/add-dir` 添加了多个工作区，先确定任务相关的工作区。
   ...
   For each task:
   ```
   $CLAUDE_PLUGIN_ROOT/bin/run-wrapper --backend ${CCG_BACKEND:-codex} - "{{WORKDIR}}" <<'EOF'
