...
| 任务类型 | 推荐角色 | 提示词路径 |
|----------|----------|-----------|
| 代码分析/理解 | analyzer | `$CLAUDE_PLUGIN_ROOT/prompts/codex/analyzer.md` |
| 架构设计/方案（默认） | architect | `$CLAUDE_PLUGIN_ROOT/prompts/codex/architect.md` |
| 问题诊断/调试 | debugger | `$CLAUDE_PLUGIN_ROOT/prompts/codex/debugger.md` |
| 性能优化 | optimizer | `$CLAUDE_PLUGIN_ROOT/prompts/codex/optimizer.md` |
| 代码审查 | reviewer | `$CLAUDE_PLUGIN_ROOT/prompts/codex/reviewer.md` |
| 测试生成 | tester | `$CLAUDE_PLUGIN_ROOT/prompts/codex/tester.md` |
...
- codeagent-wrapper 路径：`$CLAUDE_PLUGIN_ROOT/bin/run-wrapper`
...
```
Bash({
  command: "$CLAUDE_PLUGIN_ROOT/bin/run-wrapper --lite --backend ${CCG_BACKEND:-codex} - \"$(pwd)\" <<'CODEX_TASK'\nROLE_FILE: $CLAUDE_PLUGIN_ROOT/prompts/codex/<role>.md\n<TASK>\n<像对 Claude Code 说话一样的自然语言指令>\n\n项目上下文：<关键文件路径和技术栈信息>\n约束：<不可修改的文件、编码规范等>\n</TASK>\nCODEX_TASK",
...
```
Bash({
  command: "$CLAUDE_PLUGIN_ROOT/bin/run-wrapper --lite --backend ${CCG_BACKEND:-codex} resume <SESSION_ID> - \"$(pwd)\" <<'CODEX_TASK'\n<追加指令，如同人类的后续对话>\nCODEX_TASK",
  run_in_background: true,
  timeout: 3600000,
